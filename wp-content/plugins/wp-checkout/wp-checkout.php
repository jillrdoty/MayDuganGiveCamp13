	...
	class wpCheckout extends wpCheckoutPlugin {
		...
		function add_menus() {	
			...
			$this -> check_roles();
																					...
																					add_menu_page($this -> name, $this -> name . $update_icon, 'checkout_welcome', $this -> sections -> welcome, array($this, 'admin'), $this -> url() . '/images/icon-16.png');		
			...
			$this -> menus['checkout-settings-taxshipping']						=	add_submenu_page($this -> menus['checkout-settings'], __('Tax &amp; Shipping', $this -> plugin_name), __('Tax &amp; Shipping', $this -> plugin_name), 'checkout_settings_taxshipping', $this -> sections -> settings_taxshipping, array($this, 'admin_settings_taxshipping'));
			...
			$this -> menus['checkout-content'] 									= 	add_submenu_page($this -> sections -> welcome, __('Additional Content', $this -> plugin_name), __('Product Content', $this -> plugin_name), 'checkout_content', $this -> sections -> content, array($this, 'admin_content'));
			...
			$this -> menus['checkout-images']									= 	add_submenu_page($this -> sections -> welcome, __('Product Images', $this -> plugin_name), __('Product Images', $this -> plugin_name), 'checkout_images', $this -> sections -> images, array($this, 'admin_images'));
		...
		function admin_head_products_save() {
			...
			//Normal Boxes
			add_meta_box('contentsdiv', __('Additional Descriptions', $this -> plugin_name), array($this, 'contents_metabox'), "admin_page_" . $this -> sections -> products_save, 'normal', 'core');
		...
		function mceplugin($plugins) {
			...
			//Viper's Video Quicktags compatibility
			if (!empty($_GET['page']) && ($_GET['page'] == $this -> sections -> questions_save || $_GET['page'] == $this -> sections -> content_save)) {
					...
					case 'deletecoupon'						:
						if (!empty($_GET['id'])) {
							if ($order_id = $Order -> current_order_id()) {
								...
								if ($discount = $wpcoDb -> find(array('id' => $_GET['id'], 'order_id' => $order_id))) {
									$wpcoDb -> model = $Discount -> model;
										...
										if ((!empty($coupon -> expiry) && $coupon -> expiry != "0000-00-00" && $coupon -> expiry >= date("Y-m-d", time())) || empty($coupon -> expiry) || $coupon -> expiry == "0000-00-00") {
											if (empty($coupon -> maxuse) || (!empty($coupon -> maxuse) && $coupon -> used < $coupon -> maxuse)) {
												...
												if (!$wpcoDb -> find(array($co_id['type'] . '_id' => $co_id['id'], 'coupon_id' => $coupon -> id))) {												
													if (($total >= $Coupon -> discount($coupon -> id, $total)) && ($total >= ($Coupon -> discount($coupon -> id, $total) + $Discount -> total($co_id)))) {
														$wpcoDb -> model = $Discount -> model;
					...
					case 'shipping'							:											
						...
						if (!empty($_POST[$this -> pre . 'shipping'])) {
							...
							foreach ($_POST[$this -> pre . 'shipping'] as $pkey => $pval) {
								...
								$updatequery = "UPDATE `" . $wpdb -> prefix . $Order -> table . "` SET `ship_" . $pkey . "` = '" . $wpdb -> escape($pval) . "' WHERE `id` = '" . $order_id . "' LIMIT 1";
					...
					case 'payment'							:
						...
						switch ($pmethod) {
							...
							case 'pp_pro'		:
								...
								if (empty($errors)) {									
									...
									if($ack != "SUCCESS")  {
										...
										$this -> finalize_order($order_id, 'pp_pro', false, "Y");									
									}
							...
							case 'stripe'					:
								...
								$processagain = false;
								$message = __('Transaction has been processed.', $this -> plugin_name);								
								$this -> finalize_order($order -> id, "stripe", false, "Y");
								$this -> redirect($wpcoHtml -> retainquery("type=stripe&order_id=" . $order_id, $wpcoHtml -> success_url()));
							...
							case 'securetrading'		:
								...
								if (!empty($_POST)) {
									if ($_POST['errorcode'] == "0") {
										...
										$order -> pmethod = $pmethod = "securetrading";
									}
								}
								break;
							case 'payxml'			:
							...
							case 'virtualmerchant'		:							
								...
								if (!empty($_POST)) {
									if ($_POST['ssl_result'] == 0) {
										$order_id = $_POST['ssl_invoice_number'];
										if (empty($order_id) && !empty($_GET['order_id'])) { $order_id = $_GET['order_id']; }
		...
		function admin_categories() {
			...
			switch ($method) {
				...
				default					:
					...
					$order = array($wpdb -> prefix . $Category -> table . '.order', "ASC");
					...
					switch ($method) {
						case 'save'		:
							if (!empty($_POST)) {
								...
								if (!empty($product) && $product -> supplier_id == $supplier -> id) {								
									$this -> render('products' . DS . 'save');
								} else {
						...
						default			:
							...
							$data = $this -> paginate($Product -> model, "*", $this -> sections -> products, $conditions, $searchterm, $perpage);
		...
		function admin_files() {
			...
			switch ($method) {
				...
				default					:
					...
					$data = $this -> paginate($File -> model, "*", $this -> sections -> files, $conditions, $searchterm, $perpage);
		...
		function admin_suppliers() {
			...
			switch ($method) {
				...
				default					:
					...
					$data = $this -> paginate($Supplier -> model, "*", $this -> sections -> suppliers, $conditions, $searchterm, $perpage);
		...
		function admin_styles() {
			...
			switch ($method) {
				...
				default					:
					...
					$conditions = (!empty($searchterm)) ? array('title' => "LIKE '%" . $searchterm . "%'") : false;
					$data = $this -> paginate($Style -> model, "*", $this -> sections -> styles, $conditions, $searchterm, $perpage);
		...
		function admin_options() {
			...
			switch ($method) {
				...
				default					:
					...
					$conditions = (!empty($searchterm)) ? array('title' => "LIKE '%" . $searchterm . "%'") : false;
					$data = $this -> paginate($Option -> model, "*", $this -> sections -> options, $conditions, $searchterm, $perpage);
		...
		function admin_coupons() {
			...
			switch ($method) {
						...
						if (!empty($_POST['Coupon']['checklist'])) {
							...
							if ($coupons = $wpdb -> get_results($query)) {
								...
								foreach ($coupons as $coupon) {
									...
									$csvdata .= '"' . $coupon -> code . '",';
									$csvdata .= '"' . (($coupon -> discount_type == "fixed") ? __('Price', $this -> plugin_name) : __('Percentage', $this -> plugin_name)) . '",';
									$csvdata .= '"' . ($coupon -> discount) . '",';
				...
				default					:
					...
					$data = $this -> paginate($Coupon -> model, "*", $this -> sections -> coupons, $conditions, $searchterm, $perpage);
		...
		function admin_orders() {
			...
			switch ($method) {
															...
															if ($style = $wpcoDb -> find(array('id' => $style_id), array('id', 'title'))) :
																if (!empty($option_id) && is_array($option_id)) :
																	...
																	foreach ($option_ids as $option_id) :
																		$wpcoDb -> model = $Option -> model;														
																		if ($option = $wpcoDb -> find($conditions = array('id' => $option_id), $fields = array('id', 'title', 'addprice', 'price', 'operator'), array('id', "DESC"), $assign = true)) :
																			$itemsarray .= $option -> title; echo (!empty($option -> addprice) && $option -> addprice == "Y") ? ' (' . __('+', $this -> plugin_name) . $wpcoHtml -> currency_price($option -> price, true, true, $option -> operator) . ')' : '';
				...
				default						:
					...
					$data = $this -> paginate($Order -> model, "*", $this -> sections -> orders, $conditions, $searchterm, $perpage);
		...
		function admin_shipmethods() {
			...
			switch ($method) {
				...
				default					:
					...
					$data = $this -> paginate($wpcoShipmethod -> model, "*", $this -> sections -> shipmethods, $conditions, $searchterm, $perpage);
		...
		function admin_settings_taxshipping() {	
			...
			if (!empty($_POST)) {
				...
				$this -> render_msg(__('Tax &amp; shipping settings have been saved acccordingly.', $this -> plugin_name));
			}
...
//initialize the wpCheckout class
