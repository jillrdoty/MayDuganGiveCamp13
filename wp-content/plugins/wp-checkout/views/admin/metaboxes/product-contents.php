<span class="howto"><?php _e('Using additional descriptions makes use of the content tabs feature on product pages', $this -> plugin_name); ?></span>
								
<div id="<?php echo $this -> pre; ?>contents"></div>

<p><input onclick="wpco_addcontent('',''); jQuery('#<?php echo $this -> pre; ?>title' + (wpcocontent - 1)).focus();" type="button" name="addcontent" id="<?php echo $this -> pre; ?>addcontent" value="<?php _e('Add Another Description', $this -> plugin_name); ?>" class="button-secondary" /></p>
...
function wpco_addcontent(wpcotitle, wpcocontents) {					
	...
	wpcohtml += '<td><textarea rows="4" class="widefat" id="<?php echo $this -> pre; ?>textarea' + wpcocontent + '" name="Product[contents][' + wpcocontent + '][content]">' + wpcocontents + '</textarea></td>';
	wpcohtml += '</tr>';
	wpcohtml += '</tbody>';
	wpcohtml += '</table>';
	wpcohtml += '</div>';
...
jQuery(document).ready(function() {
	...
	<?php if (!empty($Product -> data -> contents)) : ?>
		<?php foreach ($Product -> data -> contents as $content) : ?>
			wpco_addcontent(<?php echo json_encode(stripslashes($content -> title)); ?>, <?php echo json_encode(stripslashes($content -> content)); ?>);
		<?php endforeach; ?>
	<?php endif; ?>
});
</script>