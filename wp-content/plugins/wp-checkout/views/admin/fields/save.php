				...
				<tr>
					...
					<td>
						...
						*/ ?>
						...
						<div style="float:left; width:49%; display:block;" id="liveproductsearchresults">
							<!-- Live Product Search Results Here -->
							<h3><?php _e('Product search results', $this -> plugin_name); ?></h3>
							<ul id="addtoproducts">
								<li id="addtoproducts_r" class="<?php echo $this -> pre; ?>lineitemdisabled"><?php _e('Search for products above and drag them to the right to assign.', $this -> plugin_name); ?></li>
							</ul>
						</div>
						...
						<div style="float:left; width:49%;">
							<h3><?php _e('Products to show this field on', $this -> plugin_name); ?></h3>
							<ul id="products">
								<li id="addtoproducts_r" class="<?php echo $this -> pre; ?>lineitemdisabled"><?php _e('Drag products here to assign custom field to them.', $this -> plugin_name); ?></li>
								<?php if (!empty($wpcoField -> data -> products)) : ?>
									<?php global $wpdb; ?>
									<?php foreach ($wpcoField -> data -> products as $product_id) : ?>
										<?php $productquery = "SELECT * FROM " . $wpdb -> prefix . $Product -> table . " WHERE id = '" . $product_id . "'"; ?>
										<?php if ($product = $wpdb -> get_row($productquery)) : ?>
											<li class="<?php echo $this -> pre; ?>lineitem" id="addtoproducts_<?php echo $product -> id; ?>"><?php echo apply_filters($this -> pre . '_product_title', $product -> title); ?></li>
										<?php endif; ?>
									<?php endforeach; ?>
								<?php endif; ?>
							</ul>
						...
						*/ ?>
						...
						jQuery(document).ready(function() {
							<?php if (!empty($wpcoField -> data -> products)) : ?>
								<?php foreach ($wpcoField -> data -> products as $product_id) : ?>
									live_product_add('<?php echo $product_id; ?>');
								<?php endforeach; ?>
							<?php endif; ?>
						});

						jQuery(document).ready(function() {
							jQuery("ul#addtoproducts").sortable({connectWith: "ul#products"});
							jQuery("ul#products").sortable({
								connectWith: "ul#addtoproducts", 
								update: function(event, ui) {
									jQuery.post(ajaxurl + '?action=wpcoaddtoproductformfields', jQuery('ul#products').sortable('serialize'), function(response) {
										jQuery('#addtoproductformfields').html(response);
									});
								}});
						});
						*/ ?>
