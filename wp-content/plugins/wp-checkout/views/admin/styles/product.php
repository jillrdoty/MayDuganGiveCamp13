	                            ...
	                            <ul id="options<?php echo $style_id; ?>" style="padding:0 0 0 80px; display:block;">
	                                ...
	                                <?php foreach ($options as $option) : ?>
	                                    <li>
	                                    	...
	                                    	<label><input onclick="jQuery('#optionstock<?php echo $option -> id; ?>div').toggle();" <?php echo (!empty($Product -> data -> options[$style_id]) && in_array($option -> id, $Product -> data -> options[$style_id])) ? 'checked="checked"' : ''; ?> type="checkbox" name="Product[options][]" value="<?php echo $option -> id; ?>" /> <?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y") ? ' (' . ((!empty($option -> operator) && $option -> operator == "curr") ? $wpcoHtml -> currency() : '') . $option -> price . ((!empty($option -> operator) && $option -> operator == "perc") ? '&#37;' : '') . ')' : ''; ?></label>
								...
								<ul id="options<?php echo $allstyle -> id; ?>" style="padding:0 0 0 80px; display:none;">
									...
									<?php foreach ($options as $option) : ?>
										<li>
											...
											<label><input onclick="jQuery('#optionstock<?php echo $option -> id; ?>div').toggle();" <?php echo (!empty($Product -> data -> options[$allstyle -> id]) && in_array($option -> id, $Product -> data -> options[$allstyle -> id])) ? 'checked="checked"' : ''; ?> type="checkbox" name="Product[options][]" value="<?php echo $option -> id; ?>" /> <?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y") ? ' (' . ((!empty($option -> operator) && $option -> operator == "curr") ? $wpcoHtml -> currency() : '') . $option -> price . ((!empty($option -> operator) && $option -> operator == "perc") ? '&#37;' : '') . ')' : ''; ?></label>
