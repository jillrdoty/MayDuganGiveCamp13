	...
	<form id="wpco_cartform" <?php if ($this -> is_plugin_active('euvatex')) : ?>onsubmit="jQuery.Watermark.HideAll();"<?php endif; ?> class="<?php echo $this -> pre; ?>" action="<?php echo $wpcoHtml -> cart_url(); ?>" method="post">	
		...
		<table class="<?php echo $this -> pre; ?> <?php echo $this -> pre; ?>cart">
			...
			<tbody>
											...
											<?php foreach ($styles as $style_id => $option_id) : ?>
												...
												<?php if ($style = $wpcoDb -> find(array('id' => $style_id), array('id', 'title'))) : ?>
														...
														<tr>
															...
															<td>
																...
																<?php foreach ($option_ids as $option_id) : ?>
																	...
																	<?php if ($option = $wpcoDb -> find(array('id' => $option_id), array('id', 'title', 'addprice', 'price', 'operator'), array('id', "DESC"), true)) : ?>
																		<?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y") ? ' (' . __('+', $this -> plugin_name) . $wpcoHtml -> currency_price($option -> price, true, true, $option -> operator) . ')' : ''; ?>
													...
													<?php else : ?>
														...
														<tr>
															<th><?php echo $style -> title; ?></th>
															<td><?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y" && !empty($option -> price)) ? ' (' . __('+', $this -> plugin_name) . $wpcoHtml -> currency_price($option -> price, true, true, $option -> operator) . ')' : ''; ?></td>
														</tr>
													<?php endif; ?>
												<?php endif; ?>
											<?php endforeach; ?>
											...
											<?php foreach ($item -> product -> cfields as $field_id) : ?>
												...
												<?php if ($field = $wpcoDb -> find(array('id' => $field_id))) : ?>
													<?php if (!empty($item -> {$field -> slug})) : ?>
														<tr>
															<th><?php echo $field -> title; ?><?php echo (!empty($field -> addprice) && $field -> addprice == "Y" && !empty($field -> price)) ? ' (' . __('+', $this -> plugin_name) . $wpcoHtml -> currency_price($field -> price, true, true) . ')' : ''; ?></th>
															<td>
				...
				<?php $subtotal = $Order -> total($co_id, false, false, true, true, false); ?>
					...
					<?php if (!empty($discounts)) : ?>
						...
						<?php foreach ($discounts as $discount) : ?>
							<tr class="total">
								...
								<td nowrap="nowrap">- <?php echo ($discount -> coupon -> discount_type == "fixed") ? $wpcoHtml -> currency_price($discount -> coupon -> discount, true, true) : $discount -> coupon -> discount; ?><?php echo ($discount -> coupon -> discount_type == "percentage") ? '&#37;' : ''; ?></td>
								<td>&nbsp;</td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
