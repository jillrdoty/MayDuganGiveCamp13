...
class wpCheckoutPlugin extends wpCheckoutCheckinit {
	...
	function ajax_chart() {
		...
		switch ($_GET['period']) {
			...
			default					:				
				...
				WHERE `completed_date` > ADDDATE(CURDATE(), INTERVAL -31 DAY) AND `completed` = 'Y' AND id IN (SELECT order_id FROM " . $wpdb -> prefix . $Item -> table . " WHERE paid = 'Y') GROUP BY DATE(completed_date)";
				...
				foreach ($orders_total as $ot) {
					$orders_count_array[date("m/d", strtotime($ot -> orders_date))] = $ot -> orders_count;
					$orders_total_array[date("m/d", strtotime($ot -> orders_date))] = $ot -> orders_total;
					$discounts_total_array[date("m/d", strtotime($ot -> orders_date))] = $ot -> discounts_total;
					$shipping_total_array[date("m/d", strtotime($ot -> orders_date))] = $ot -> shipping_total;
					$tax_total_array[date("m/d", strtotime($ot -> orders_date))] = $ot -> tax_total;
				}	
				
				for ($i = 0; $i <= 31; $i++) {
					$datestring = date("m/d", strtotime("-" . $i . " days"));
					$dates_array[] = date("M j", strtotime("-" . $i . " days"));
				...
				$g->set_tool_tip( '#key#: ' . html_entity_decode($wpcoHtml -> currency()) . '#val#<br>Date: #x_label#' );
				...
				//$g->set_y_legend( 'Amount in ' . html_entity_decode($wpcoHtml -> currency()), 12, '#000000');
	...
	function enqueue_scripts() {
		...
		if (is_admin()) {						
			...
			$datefieldquery = "SELECT COUNT(`id`) FROM " . $wpdb -> prefix . $wpcoField -> table . " WHERE `type` = 'pre_date'";
			if ($wpdb -> get_var($datefieldquery)) {
			...
			wp_enqueue_script('msdropdown', $this -> render_url('js/jquery.dd.js', 'default', false), array('jquery'), false, true);
			
			$filefieldquery = "SELECT COUNT(`id`) FROM " . $wpdb -> prefix . $wpcoField -> table . " WHERE `type` = 'file'";
	...
	function enqueue_styles() {	
		...
		if (is_admin()) {
			...
			if ($this -> get_option('theme_usestyle') == "Y") {
				$src = plugins_url() . '/' . $this -> plugin_name . '/views/' . $theme_folder . '/style.css';
			}
			
			$colorbox_src = plugins_url() . '/' . $this -> plugin_name . '/views/' . $theme_folder . '/css/colorbox.css';
			$msdropdown_src = plugins_url() . '/' . $this -> plugin_name . '/views/' . $theme_folder . '/css/dd.css';
			$jqueryui_src = plugins_url() . '/' . $this -> plugin_name . '/views/' . $theme_folder . '/css/jquery-ui.css';
		}
		...
		wp_enqueue_style('dd', $msdropdown_src, null, $this -> version, "screen");
	...
	function updating_plugin() {		
		...
		if ($cur_version = $this -> get_option('version')) {
			...
			if (version_compare($cur_version, "1.6.9.4") < 0) { 
				$this -> initialize_options();
				$new_version = "1.6.9.4";
			}
	...
	function initialize_options() {	
		...
		$this -> add_option('captions', $captions);
			...
			if ($user_ID) {
				...
				if ($file = $wpcoDb -> find(array('id' => $file_id))) {
					...
					if ($product = $wpcoDb -> find(array('id' => $file -> product_id))) {
						...
						if ($items = $wpcoDb -> find_all(array('product_id' => $product -> id, 'completed' => "Y", 'paid' => "Y", 'user_id' => $user_ID))) {
							if (!empty($items)) {
								return true;
							}
						}
	...
	function has_downloads($user_id = null) {
		...
		if (!empty($id)) {
			...
			if ($items = $wpcoDb -> find_all(array('completed' => "Y", 'paid' => "Y", 'user_id' => $id))) {
				foreach ($items as $item) {
					$wpcoDb -> model = $File -> model;
					
					if ($files = $wpcoDb -> find_all(array('product_id' => $item -> product -> id))) {
						return true;
					}
				}
			}
					...
					<fieldset class="<?php echo $this -> pre; ?> <?php echo $this -> pre; ?>variation" id="<?php echo $this -> pre; ?>variation<?php echo $style_id; ?>">
						...
						switch ($style -> type) {
							case 'select'				:							
								...
								<select id="Item_style<?php echo $style_id; ?><?php echo $product_id; ?>" onchange="wpco_updateproductprice('<?php echo $product_id; ?>', '<?php _e('Calculating...', $this -> plugin_name); ?>');" class="<?php echo $this -> pre; ?>style_select" name="Item[styles][<?php echo $style_id; ?>]">
									...
									<?php foreach ($options as $option_id) : ?>
										...
										<?php if ($option = $wpcoDb -> find(array('id' => $option_id))) : ?>
											<?php if (!empty($option -> title)) : ?>
												<option <?php if (!empty($option -> image)) : ?>title="<?php echo $wpcoHtml -> timthumb_image_src($option -> image_url, $this -> get_option('variations_optionthumbw'), $this -> get_option('variations_optionthumbh'), 100); ?>"<?php endif; ?> <?php echo ($inventory == 0) ? 'disabled="disabled"' : ''; ?> <?php echo ((!empty($_POST['Item']['styles'][$style_id]) && $_POST['Item']['styles'][$style_id] == $option_id) || (!empty($productsstyle -> defaultoption) && $productsstyle -> defaultoption == $option -> id)) ? 'selected="selected"' : ''; ?> value="<?php echo $option -> id; ?>"><?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y" && $option -> condprices == "N" && $option -> price > 0) ? ' (+' . ((!empty($option -> operator) && $option -> operator == "curr") ? $wpcoHtml -> currency() : '') . $option -> price . ((!empty($option -> operator) && $option -> operator == "perc") ? '&#37;' : '') . ')' : ''; ?><?php echo (!empty($showstock) && $showstock == true && $inventory > 0) ? ' (' . $inventory . ' ' . __('in stock', $this -> plugin_name) . ')' : ''; ?><?php echo ($inventory == "0") ? ' (' . __('out of stock', $this -> plugin_name) . ')' : ''; ?></option>
											<?php endif; ?>
										<?php endif; ?>
									<?php endforeach; ?>
								</select>
                                ...
                                jQuery(document).ready(function() {
                                	if (jQuery.isFunction(jQuery.fn.msDropDown)) {
                                		jQuery('#Item_style<?php echo $style_id; ?><?php echo $product_id; ?>').msDropDown();
                                	}
                                });
                                </script>
								...
								<?php foreach ($options as $option_id) : ?>
									...
									<?php if ($option = $wpcoDb -> find(array('id' => $option_id), array('id', 'title', 'image', 'addprice', 'price', 'operator', 'condprices'))) : ?>                                    
										...
										<?php if (!empty($option -> image)) : ?>
                                            <label class="<?php echo $this -> pre; ?>style_radio <?php echo $this -> pre; ?>style_withimage <?php echo $this -> pre; ?>style_float">
                                                ...
                                                <?php if ($this -> get_option('variations_optionlabel') == "Y") : ?>
                                                    <span class="<?php echo $this -> pre; ?>style_optionlabel"><?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y" && $option -> condprices == "N") ? ' <span class="' . $this -> pre . 'option_price">(+' . ((!empty($option -> operator) && $option -> operator == "curr") ? $wpcoHtml -> currency() : '') . $option -> price . ((!empty($option -> operator) && $option -> operator == "perc") ? '&#37;' : '') . ')</span>' : ''; ?></span>
                                        ...
                                        <?php else : ?>
                                            ...
                                            <?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y" && $option -> condprices == "N") ? ' <span class="' . $this -> pre . 'option_price">(+' . ((!empty($option -> operator) && $option -> operator == "curr") ? $wpcoHtml -> currency() : '') . $option -> price . ((!empty($option -> operator) && $option -> operator == "perc") ? '&#37;' : '') . ')</span>' : ''; ?>
                                            <span class="<?php echo $this -> pre; ?>option_stock"><?php echo (!empty($showstock) && $showstock == true && $inventory > 0) ? ' (' . $inventory . ' ' . __('in stock', $this -> plugin_name) . ')' : ''; ?><?php echo ($inventory == "0") ? ' (' . __('out of stock', $this -> plugin_name) . ')' : ''; ?></span>
                                            </label>
                                        <?php endif; ?>
									<?php endif; ?>
									...
									<?php if ($option = $wpcoDb -> find(array('id' => $option_id), array('id', 'title', 'image', 'addprice', 'price', 'operator', 'condprices'))) : ?>
                                    	...
                                    	<?php if (!empty($option -> image)) : ?>
                                        	<label class="<?php echo $this -> pre; ?>style_checkbox <?php echo $this -> pre; ?>style_withimage <?php echo $this -> pre; ?>style_float">
                                                ...
                                                <?php if ($this -> get_option('variations_optionlabel') == "Y") : ?>
                                                	<span class="<?php echo $this -> pre; ?>style_optionlabel"><?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y" && $option -> condprices == "N") ? ' <span class="' . $this -> pre . 'option_price">(+' . $wpcoHtml -> currency_price($option -> price, true, true) . ')</span>' : ''; ?></span>
                                                	<span class="<?php echo $this -> pre; ?>option_stock"><?php echo (!empty($showstock) && $showstock == true && $inventory > 0) ? ' (' . $inventory . ' ' . __('in stock', $this -> plugin_name) . ')' : ''; ?><?php echo ($inventory == "0") ? ' (' . __('out of stock', $this -> plugin_name) . ')' : ''; ?></span>
                                                <?php endif; ?>
                                            </label>
                                            <?php if ($this -> get_option('variations_optionlabel') == "Y") : ?>
                                        ...
                                        <?php else : ?>
                                        	...
                                        	<input <?php echo ($inventory == 0) ? 'disabled="disabled"' : ''; ?> onclick="wpco_updateproductprice('<?php echo $product_id; ?>', '<?php _e('Calculating...', $this -> plugin_name); ?>');" type="checkbox" <?php echo ((!empty($_POST['Item']['styles'][$style_id]) && in_array($option_id, $_POST['Item']['styles'][$style_id])) || (!empty($productsstyle -> defaultoption) && $productsstyle -> defaultoption == $option_id) && $inventory != 0) ? 'checked="checked"' : ''; ?> class="<?php echo $this -> pre; ?>style_checkbox_input" name="Item[styles][<?php echo $style_id; ?>][]" value="<?php echo $option -> id; ?>" /> <?php echo $option -> title; ?><?php echo (!empty($option -> addprice) && $option -> addprice == "Y" && $option -> condprices == "N") ? ' <span class="' . $this -> pre . 'option_price">(+' . ((!empty($option -> operator) && $option -> operator == "curr") ? $wpcoHtml -> currency() : '') . $option -> price . ((!empty($option -> operator) && $option -> operator == "perc") ? '&#37;' : '') . ')</span>' : ''; ?>
                                        	<span class="<?php echo $this -> pre; ?>option_stock"><?php echo (!empty($showstock) && $showstock == true && $inventory > 0) ? ' (' . $inventory . ' ' . __('in stock', $this -> plugin_name) . ')' : ''; ?><?php echo ($inventory == "0") ? ' (' . __('out of stock', $this -> plugin_name) . ')' : ''; ?></span>
                                            </label>
                                        <?php endif; ?>
									<?php endif; ?>
	...
	function render_field($field_id = null, $fieldset = true, $value = null, $product_id = null, $globalfield = false) {
		...
		if (!empty($field_id)) {
			...
			if ($field = $wpcoDb -> find(array('id' => $field_id))) {			
				...
				switch ($field -> type) {
					case 'text'				:
						echo '<input onkeyup="' . $jsoutput . '" id="' . $field -> slug . $product_id . '" type="text" name="Item[fields][' . $field -> id . ']' . '" value="' . $fieldvalue . '" />';
						break;
					case 'file'				:
						//echo '<input type="file" name="Item[fields][' . $field -> id . ']" id="' . $field -> slug . $product_id . '" class="wpco wpcofield wpcofield-file" />';
						/*$filetypes = false;
						...
						break;
					case 'textarea'			:
						echo '<textarea onkeyup="' . $jsoutput . '" id="' . $field -> slug . $product_id . '" rows="3" cols="98%" style="width:98%;" name="Item[fields][' . $field -> id . ']' . '">' . strip_tags(esc_attr($fieldvalue)) . '</textarea>';
						break;
					case 'select'			:
						echo '<select onchange="' . $jsoutput . '" id="' . $field -> slug . $product_id . '" name="Item[fields][' . $field -> id . ']' . '">';
					...
					case 'checkbox'			:
						...
						break;
					case 'pre_date'			:
						echo '<input onblur="' . $jsoutput . '" id="' . $field -> slug . $product_id . '" class="datepicker" type="text" name="Item[fields][' . $field -> id . ']' . '" value="' . $fieldvalue . '" />';
	...
	function render_url($file = null, $folder = 'admin', $extension = null) {	
		if (!empty($file)) {		
			if (!empty($folder) && $folder == "default") {
				...
				$theme_name = basename($template_url);
				$theme_path = $theme_root . DS . $theme_name;
	...
	function render($file = null, $params = array(), $output = true, $folder = 'admin', $extension = false) {
		...
		if (!empty($file)) {
			...
			if (file_exists($filefull)) {
				...
				$captions = $this -> get_option('captions');
