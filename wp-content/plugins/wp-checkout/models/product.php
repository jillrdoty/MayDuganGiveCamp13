...
class wpcoProduct extends wpCheckoutPlugin {
	...
	var $fields_tv = array(
		...
		'hits'				=>	array("INT(11)", "NOT NULL DEFAULT '0'"),
	...
	function wpcoProduct($data = array()) {
		...
		if (!empty($data)) {		
			...
			if ($this -> recursive == true) {
				...
				$this -> cfields = array();
				$fieldsquery = "SELECT `id` FROM " . $wpdb -> prefix . $wpcoField -> table . " ORDER BY `order` ASC";
				if ($fields = $wpdb -> get_results($fieldsquery)) {
					foreach ($fields as $field) {
						$fieldsproductquery = "SELECT `id` FROM " . $wpdb -> prefix . $wpcoFieldsProduct -> table . " WHERE `product_id` = '" . $this -> id . "' AND `field_id` = '" . $field -> id . "'";
						if ($fieldsproduct = $wpdb -> get_row($fieldsproductquery)) {
							$this -> cfields[] = $field -> id;
						}
					}
				}				
	...
	function get($product_id = null) {
		...
		if (!empty($product_id)) {
			$query = "SELECT * FROM `" . $wpdb -> prefix . "" . $this -> table . "` WHERE `status` = 'active' and id` = '" . $product_id . "'";
		...
		if (!empty($formvalues['Item'])) {
			...
			if (!empty($item['product_id'])) {
				...
				if ($product = $wpcoDb -> find(array('id' => $item['product_id']))) {								
					...
					if ($this -> get_option('tax_includeinproductprice') == "Y" && $includetax == true) {					
				...
				/* Tiered price products */
					...
					if ($tiers = @unserialize($product -> price)) {
						...
						if (!empty($product -> vtreatasone) && $product -> vtreatasone == "Y") {
							...
							$allitems_query = "SELECT SUM(`count`) FROM `" . $wpdb -> prefix . "" . $Item -> table . "` WHERE `product_id` = '" . $product -> id . "' AND `order_id` = '" . $item -> order_id . "';";
			...
			if ($wpdb -> query($query)) {
					...
					foreach ($contents as $content) {
						if (!empty($content['title']) && !empty($content['content'])) {
							$c_data = array(
								'wpcoContent'	=>	array(
									'title'			=>	$content['title'],
									'content'		=>	$wpdb -> escape($content['content']),
				...
				if (!empty($fields)) {					
					foreach ($fields as $field_id) {
						$wpcoDb -> model = $wpcoFieldsProduct -> model;
						$wpcoDb -> save(array('product_id' => $this -> insertid, 'field_id' => $field_id), true);
					}
				}
				...
				if (!empty($categories)) {				
					foreach ($categories as $c_id) {
						$wpcoDb -> model = $wpcoCategoriesProduct -> model;
						$cp_data = array('category_id' => $c_id, 'product_id' => $this -> insertid);
						$wpcoDb -> save($cp_data);
					}
				}
