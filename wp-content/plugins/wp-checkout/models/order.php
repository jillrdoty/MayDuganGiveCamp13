...
class wpcoOrder extends wpCheckoutPlugin {
	...
	function current_order_id($autocreate = false) {	
		...
		//$query1 = "SELECT `id` FROM `" . $wpdb -> prefix . $this -> table . "` WHERE `user_id` = '" . $user_ID . "' AND `completed` = 'N'";
	...
	function update_totals($o_id = null) {
		...
		if (!empty($order_id)) {
			...
			$shipping = $this -> shipping_total($total, $co_id);
	...
	function total($co_id = null, $shipping = false, $applydiscount = true, $styles = true, $fields = true, $tax_calculate = true) {
									...
									foreach ($ustyles as $ustyle => $uoption) {
										if (!empty($uoption) && is_array($uoption)) {
											...
											foreach ($uoptions as $uoid) {
												$total += ($Option -> price($uoid, false, $producttotal) * $item -> count);
											}
										} else {
											$total += ($Option -> price($uoption, $ustyles, $producttotal) * $item -> count);
										}
									}
								}
							}
