...
class wpcoCategory extends wpCheckoutPlugin {
	...
	function field($name = '', $conditions = array()) {
		global $wpdb;
	...
	function save($data = array(), $validate = true, $savepp = true) {
		...
		if ($validate == true) {
			...
			if ($useimage == "Y") {
				...
				if (!empty($oldimage) && empty($image -> name)) {
					...
					//elseif (file_exists($filefull)) { $this -> errors['image'] = __('File with same file name already in use', $this -> plugin_name); }
					...
					//elseif (!$this -> image_resize($imagefull, $this -> get_option('catthumbw'), $this -> get_option('catthumbh'), $cropcatthumb, $append = 'thumb', $dest_path, $quality = 100)) { $this -> errors['image'] = __('Thumbnail could not be created', $this -> plugin_name); }
					
					//$this -> data -> image -> name = $wpcoHtml -> sanitize($title) . '.' . strtolower($wpcoHtml -> strip_ext($image -> name));
				}
			} else {
				$this -> data -> image = '';
			}
		...
		if (empty($this -> errors)) {
			if ($useimage == "Y") {
				//$this -> image_resize($imagefull, $width = 50, $height = 50, $crop = true, $append = 'small', $dest_path, $quality = 100);
				$this -> data -> image = $this -> data -> image -> name;
				
				//$nam = $wpcoHtml -> strip_ext($this -> data -> image -> name, 'nam');
				//$ext = strtolower($wpcoHtml -> strip_ext($this -> data -> image -> name, 'ext'));
				//$thumbfull = $imagepath . $nam . '-thumb.' . $ext;
				//$smallfull = $imagepath . $nam . '-small.' . $ext;
